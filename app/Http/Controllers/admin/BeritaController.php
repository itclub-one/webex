...
class BeritaController extends Controller
{
        ...
        return DataTables::of($data)
            ->addColumn('action', function ($row) {
                ...
                if (isAllowed(static::$module, "edit")) : //Check permission
                    $btn .= '<a href="'.route('admin.berita.edit',$row->id).'" class="btn btn-primary btn-sm mx-3 ">
                    Edit
                </a>';
                endif;
                if (isAllowed(static::$module, "detail")) : //Check permission
                    $btn .= '<a href="#" data-id="' . $row->id . '" class="btn btn-secondary btn-sm " data-toggle="modal" data-target="#detailBerita">
                    Detail
                </a>';
                endif;
                return $btn;
    ...
    public function add(){
        ...
        return view('administrator.berita.add');
    }
    ...
    public function save(Request $request){
        ...
        if ($request->hasFile('img_url')) {
            ...
            if (!empty($data->img_url)) {
                $image_path = "./administrator/assets/media/berita/" . $data->img_url;
            ...
            $fileName = $data->judul . '_' . date('Y-m-d-H-i-s') . '_' . uniqid(2) . '.' . $image->getClientOriginalExtension();
            $path = upload_path('berita') . $fileName;
    ...
    public function update(Request $request)
    {
        ...
        if ($request->hasFile('img_url')) {
            if (!empty($data->img_url)) {
                ...
                $fileName = $data->judul . '_' . date('Y-m-d-H-i-s') . '_' . uniqid(2) . '.' . $request->file('img_url')->getClientOriginalExtension();
                $path = upload_path('berita') . $fileName;
