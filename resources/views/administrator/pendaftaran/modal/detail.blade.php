<!-- Modal Detail Pendaftaran Anggota -->
<div class="modal fade" tabindex="-1" role="dialog" id="detailPendaftaran" data-backdrop="false">
    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="detailPendaftaranLabel">Detail Pendaftaran Anggota</h5>
                <button type="button" class="close" id="buttonCloseModuleModal" data-dismiss="modal"
                    aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body" id="detailPendaftaranBody">
...
@push('js')
    <script>
        $('#detailPendaftaran').on('show.bs.modal', function(event) {
            ...
            var modalBody = $('#detailPendaftaranBody');
            ...
            $.ajax({
                url: '{{ route('admin.pendaftaran.getDetail', ':id') }}'.replace(':id', id),
                method: 'GET',
                success: function(response) {
                    ...
                    modalBody.html(
                        ...
                        '<div class="row mt-4">' +
                        '<div class="col-12">' +
                        '<div class="title"><strong>Alasan Masuk</strong> :</div>' +
                        '</div>' +
                        '<div class="col-12">' + data.alasan_masuk + '</div>' +
                        '</div>' 
                    );
