...
@section('content')
            ...
            <div class="card">
                <div class="card-header">
                    ...
                    <div class="col-4" style="display: flex; justify-content: flex-end;">
                        @if (isallowed('dokumentasi', 'add'))
                            <a href="{{ route('admin.dokumentasi.add') }}" class="btn btn-primary mx-1">Tambah Data</a>
                        @endif
                        <a href="javascript:void(0)" class="btn btn-primary mx-1" id="filterButton">Filter</a>
                    </div>
                    
                </div>
                @include('administrator.dokumentasi.filter.main')
                <div class="card-body">
    ...
    @include('administrator.dokumentasi.modal.detail')
@endsection
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            var data_table = $('#datatable').DataTable({
                ...
                ajax: {
                    url: '{{ route('admin.dokumentasi.getData') }}',
                    dataType: "JSON",
                    type: "GET",
                    data: function(d) {
                        d.eskul = getEskul();
            ...
            $(document).on('click', '.delete', function(event) {
                ...
                swalWithBootstrapButtons.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            type: "DELETE",
                            url: "{{ route('admin.dokumentasi.delete') }}",
                            data: {
                                "_token": "{{ csrf_token() }}",
                                "_method": "DELETE",
                                "id": id,
                            ...
                            success: function() {
                                // data_table.ajax.url(
                                //         '{{ route('admin.dokumentasi.getData') }}')
                                //     .load();
            ...
            $('#filter_submit').on('click', function(event) {
                ...
                data_table.ajax.url('{{ route('admin.dokumentasi.getData') }}?eskul=' + filterEskul )
                    .load();
            });
